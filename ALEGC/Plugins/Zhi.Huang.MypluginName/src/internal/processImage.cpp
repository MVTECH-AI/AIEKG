...
void ImageProcess::testSVMs(ReadData *pst)
{
	...
	for (int i = 0; i < pcaTestDst.rows; i++){
		result = -1;
...
int ImageProcess::predictSVMs(ReadData *pst,QString getDataName,Mat realMat)
{
	...
	realpredictData = realMat.reshape((1,1));
	//realpredictData = asRowMatrix(pcaRealMat,CV_32FC1);
	qDebug() <<realpredictData.rows << pcaData.cols;
	...
	ofstream writeData(getDataName.toStdString().c_str());
...
void ImageProcess::beginTransmission(ReadData *pst,QString getDataName)
{
	QTcpSocket *client = new QTcpSocket;
	client->connectToHost("192.168.1.183",12345);
	ifstream inData(getDataName.toStdString().c_str());
	int length;
	...
	inData.close();
	client->write(getDataName.toStdString().c_str());
	client->write(" ");
